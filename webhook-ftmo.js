...
const RISK_PERCENT    = 0.005;                           // 0.5% per trade
const RISK_EUR_BASE   = ACCOUNT_BALANCE * RISK_PERCENT; // €50 bij €10k
const RISK_EUR_MAX    = RISK_EUR_BASE * 1.8;            // ~€90 fallback
  ...
  if (type === "stock") {
    ...
    if (lots < 1) {
      ...
      if (riskWith1 <= RISK_EUR_MAX) {
        console.log(`⬆️ 1 share = €${riskWith1.toFixed(2)} ≤ €${RISK_EUR_MAX.toFixed(2)} — doorgaan`);
        lots = 1;
      } else {
        console.warn(`❌ 1 share = €${riskWith1.toFixed(2)} > €${RISK_EUR_MAX.toFixed(2)} — geannuleerd`);
        return null;
      }
    }
    ...
    if (lots < lotStep) {
      ...
      if (riskWithMin <= RISK_EUR_MAX) {
        console.log(`⬆️ Min lot = €${riskWithMin.toFixed(2)} ≤ €${RISK_EUR_MAX.toFixed(2)} — doorgaan`);
        lots = lotStep;
      } else {
        console.warn(`❌ Min lot = €${riskWithMin.toFixed(2)} > €${RISK_EUR_MAX.toFixed(2)} — geannuleerd`);
        return null;
      }
    }
    lots = Math.min(maxLots, lots);
