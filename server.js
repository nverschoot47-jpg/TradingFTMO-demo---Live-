...
async function finalizeGhost(ghost) {
  ...
  openPositions.delete(ghost.positionId);
...
async function syncPositions() {
  ...
  try {
    ...
    // 1. Detect closed MT5 positions
    for (const [id, pos] of openPositions) {
      if (liveIds.has(id)) continue;
      // Position gone from MT5 — determine why
      let closeReason = "sl";
      try {
      ...
      await db.saveClosedTrade({
        ...
        exitPrice:      null, // populated from deal price if available
        closeReason,
      ...
      if (closeReason === "sl") {
        ...
        // MT5 TP: ghost keeps running. Mark it and keep in openPositions.
        if (ghost) {
          ghost.mt5ClosedTP    = true;
          ghost.mt5CloseAt     = new Date().toISOString();
          ghost.mt5CloseReason = "tp";
          // Remove MT5 position tracking but keep ghost
          pos.mt5Closed = true;
    ...
    // 3. Also update ghost-only positions (MT5 closed on TP, ghost still running)
    for (const [id, pos] of openPositions) {
      if (!pos.mt5Closed || !pos.ghost) continue;
      // Need current price from somewhere — use last known or skip
      // In practice the position is gone from MT5 so we can't get live price.
      // We'll rely on the next time it appears in MT5 history or just wait.
    }
...
async function adoptPosition(lp) {
  ...
  const optimizerKey = buildOptimizerKey(symbol, session, "unknown", "unknown");
...
app.post("/webhook", async (req, res) => {
  ...
  if (!allowed) {
    await db.logSignal({
      ...
      slPct: sl_pct ? parseFloat(sl_pct) : null,
      latencyMs: Date.now() - t0,
      slPoints:    sl_points    ? parseFloat(sl_points)    : null,
      vwapMid:     vwap         ? parseFloat(vwap)         : null,
      vwapUpper:   vwap_upper   ? parseFloat(vwap_upper)   : null,
      vwapLower:   vwap_lower   ? parseFloat(vwap_lower)   : null,
      sessionHigh: session_high ? parseFloat(session_high) : null,
      sessionLow:  session_low  ? parseFloat(session_low)  : null,
      dayHigh:     day_high     ? parseFloat(day_high)     : null,
      dayLow:      day_low      ? parseFloat(day_low)      : null,
    });
...
async function loadGhostTracker(){
  ...
  // Build headers with max fav = 5.0
  const MAX_FAV=5.0;
  const hdrCells='<th>Status</th><th>#</th><th>Symbol</th><th>MT5 Comment</th><th>Type</th><th>Dir</th><th>VWAP</th><th>Session</th><th style="color:#388bfd">RR Now</th><th style="color:#3fb950">Peak+</th><th style="color:#f85149">Peak−</th><th>TP Set</th>'+buildMsHeaders(MAX_FAV)+'<th>TV Entry</th><th>Entry</th><th style="color:#f85149">SL</th><th style="color:#3fb950">TP</th><th>Lots</th><th>Opened</th><th style="color:#d29922">VWAP</th><th>VWAP+</th><th>VWAP−</th><th style="color:#d29922">S.High</th><th style="color:#d29922">S.Low</th><th style="color:#39d3f2">D.High</th><th style="color:#39d3f2">D.Low</th><th>Band%</th><th>SL%</th><th>SL pts</th>';
  const hdrEl=$('gh-ms-header');if(hdrEl)hdrEl.innerHTML=hdrCells;
  const body=$('gh-active-body');if(!body)return;
  if(!_pos.length){body.innerHTML='<tr><td colspan="50" class="nd">No active ghost trades</td></tr>';return;}
  body.innerHTML=_pos.map(p=>{
    const g=p.ghost||{};
    const ms=g.rrMilestones||{};
    const rr=rrFromPrice(p.entry,p.sl,p.currentPrice,p.direction);
    const pkp=g.peakRRPos||0,pkn=g.peakRRNeg||0;
    const isIdx=p.assetType==='index';
    return '<tr><td>'+(g.mt5ClosedTP?'<span class="bd" style="background:rgba(188,140,255,.15);color:#bc8cff;border:1px solid rgba(188,140,255,.3)">GHOST</span>':'<span class="bd bd-live">● LIVE</span>')+'</td><td class="cw fw" style="font-size:9px">'+(p.dailyLabel||'--')+'</td><td class="cw fw">'+p.symbol+'</td><td class="cd" style="font-size:8px">'+(p.mt5Comment||'--')+'</td><td>'+bdType(p.assetType)+'</td><td>'+bdDir(p.direction)+'</td><td>'+bdVwap(p.vwapPosition)+'</td><td>'+bdSess(p.session)+'</td><td>'+rrHtml(rr)+'</td><td>'+(pkp>0?'<span class="cg fw">+'+pkp.toFixed(2)+'R</span>':'--')+'</td><td>'+(pkn>0?'<span class="cr">-'+(pkn/100).toFixed(2)+'R</span>':'--')+'</td><td class="cg">+1.50R</td>'+buildMsRow(ms,MAX_FAV)+'<td class="cd" style="font-size:9px">'+fmt(p.tvEntry,isIdx?2:5)+'</td><td class="cw">'+fmt(p.entry,isIdx?2:5)+'</td><td class="cr">'+fmt(p.sl,isIdx?2:5)+'</td><td class="cg">'+fmt(p.tp,isIdx?2:5)+'</td><td class="cd">'+fmt(p.lots,2)+'</td><td class="cd" style="font-size:9px">'+fmtTs(p.openedAt)+'</td><td class="cy" style="font-size:9px">'+(p.vwapMid!=null?fmt(p.vwapMid,isIdx?2:5):'--')+'</td><td class="cd" style="font-size:9px">'+(p.vwapUpper!=null?fmt(p.vwapUpper,isIdx?2:5):'--')+'</td><td class="cd" style="font-size:9px">'+(p.vwapLower!=null?fmt(p.vwapLower,isIdx?2:5):'--')+'</td><td class="cy" style="font-size:9px">'+(p.sessionHigh!=null?fmt(p.sessionHigh,isIdx?2:5):'--')+'</td><td class="cy" style="font-size:9px">'+(p.sessionLow!=null?fmt(p.sessionLow,isIdx?2:5):'--')+'</td><td class="cb2" style="font-size:9px">'+(p.dayHigh!=null?fmt(p.dayHigh,isIdx?2:5):'--')+'</td><td class="cb2" style="font-size:9px">'+(p.dayLow!=null?fmt(p.dayLow,isIdx?2:5):'--')+'</td><td class="cd">'+(p.vwapBandPct!=null?Number(p.vwapBandPct).toFixed(1)+'%':'--')+'</td><td class="cd">'+(p.slPct!=null?(p.slPct*100).toFixed(3)+'%':'--')+'</td><td class="cd">'+(p.slPoints!=null?Number(p.slPoints).toFixed(2):'--')+'</td></tr>';
  }).join('');
}
