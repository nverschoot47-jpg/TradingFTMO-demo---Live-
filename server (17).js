...
// Cleanup: remove finalized ghosts from memory after 30 minutes
// They stay in DB forever — just cleaned from in-memory map
function cleanupFinalizedGhosts() {
  const cutoff = Date.now() - 30 * 60 * 1000; // 30 min
  for (const [id, pos] of openPositions) {
    if (pos.ghostFinalized && pos.finalizedAt < cutoff) {
  ...
  try {
    ...
    await Promise.all(closedIds.map(async id => {
      ...
      if (pos._exitPrice && pos.tp && pos.sl) {
        ...
        if (ghost && ghost.peakRRPos >= 1.45) closeReason = "tp";
      } else if (pos.ghost && pos.ghost.peakRRPos >= 1.45) {
...
async function loadOverview(){
  ...
  for(const t of _cl){
    ...
    const isTP = t.closeReason==='tp' || (t.peakRRPos >= 1.45);
    const isSL = !isTP;
...
async function loadGhostTracker(){
  ...
  body.innerHTML=_pos.map(p=>{
    ...
    // Status badge: finalized → SL badge (ghost always ends on SL)
    // MT5 TP but ghost still running → GHOST badge (purple)
    // Live → LIVE badge (green)
    let statusBadge;
    if(p.ghostFinalized){
      statusBadge='<span class="bd bd-sl" style="padding:2px 7px;font-size:9px;font-weight:700">SL</span>';
